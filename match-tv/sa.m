...
%
% The data are generated from digital photographs of matches
% by slicing the photos into small fixed-size patches.

close all;
clear all;

% Save console output to file
diary('match-output.txt');
...
n_accrate = 100;        % number of last proposals used to compute acceptance rate
alpha = 0.97;           % decrement coefficient for temperature
...
t0 = 38.0;  % initial temperature
t1 = 0.1;
t2 = 0.05;
cd(data);
imagedata = dir('*.tif');
n_img = length(imagedata);
img = cell(1,n_img);
for i = 1:n_img
...
save('match-final.mat', 'finals');

% End-of-chain errors
save('match-eoc-l2.mat','err_eoc');
save('match-eoc-ssim.mat','ssim_eoc');

% Window showing algorithm progress and final state
print('match-plot','-dpng'); % not optimised
